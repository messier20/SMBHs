...
class NFW:
    ...
    def calc_mass(self, total_mass, dot_radius, dotdot_radius, halo_scale, radius_scaled, radius):
        ...
        # print((dot_radius ** 2) / (
        #         halo_scale ** 2) * (1. - radius_scaled) / ((1 + radius_scaled) ** 3), ' paskutinis narys po +')
        # print(self.concentration_member, ' conc member')
        # print(third_mddt, ' third member')
        # TODO figure out why in idl code ! was written before pi
        density = self.Calculator.calc_density(total_mass, 4., halo_scale, self.radius_scaled_squared_member,
